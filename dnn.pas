...
EDNNLError =class (Exception)
end;

procedure wrap_c_api(const status:dnnl_status_t; const message :string);
{$define DNNL_THROW_ERROR:=wrap_c_api}


/// @cond DO_NOT_DOCUMENT_THIS


procedure validate_container_size<T>(const v:T; const error_message:string; min_size:integer = 1; max_size:integer = -1);
...
private
    ...
    class function dummy_destructor():dnnl_status_t; static;{ return dnnl_success; }
...
public
    ...
    /// Copy constructor.
    (* handle(const hande<T, traits> &) = defaulr;  *)
    ...
    ///     defaults to @c false.
    constructor create(t:T;  weak:boolean = false); { reset(t, weak); }
    ...
    ///     defaults to @c false.
    procedure reset(t:T; weak :boolean= false); {
    ...
    /// @returns The underlying C API handle.
    function get(allow_empty:boolean = false):T; {
        T result = data_.get();
...
//handle_traits=handle<dnnl_memory_t>;
//handle_traitsHelper = type helper for handle_traits
//class function destroy(p :dnnl_memory_t):dnnl_status_t;static;
//(*{
...
Memory=handle<dnnl_memory_t>;
MemoryHelper = type helper for Memory;
  class function destroy(p:dnnl_memory_t):dnnl_status_t;static;//  return dnnl_memory_destroy(p);
end;
...
procedure wrap_c_api(const status:dnnl_status_t; const message :string);
const err:array[0..6] of string =  ('dnnl_success', 'dnnl_out_of_memory', 'dnnl_invalid_arguments', 'dnnl_unimplemented','dnnl_iterator_ends', 'dnnl_runtime_error','dnnl_not_required');
begin
  if status<>dnnl_succsess then
    raise EDNNLError.create(Uppercase(err[status])+': '+message);
end;
...
begin
    size := v.size();
    if (size < min_size) or (max_size >= 0) and (size > max_size) then
